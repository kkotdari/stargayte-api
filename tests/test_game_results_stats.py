...
async def test_stats_counts_registered_matches(client):
    """등록한 경기 수(registered) — 통계 화면의 "기록 퀸"이 쓰는 값(요청).

    뛴 것이 아니라 올린 것이라, 남의 경기를 대신 올려 준 사람에게도 쌓인다. 그게 이 값의
    뜻이다: 클럽의 기록을 누가 남기고 있나. 기간·유형 필터는 전적과 똑같이 탄다.
    """
    p1 = await _signup(client, "player01", "Shadow#1001")
    p2 = await _signup(client, "player02", "Mist#1002")
    h1 = {"Authorization": f"Bearer {p1['accessToken']}"}
    h2 = {"Authorization": f"Bearer {p2['accessToken']}"}
    # 첫 가입자만 바로 활동 상태다 — 둘째는 승인해야 글을 올릴 수 있다.
    res = await client.patch("/api/members/player02/status", headers=h1, json={"status": "active"})
    assert res.status_code == 200, res.text

    # player01이 두 건, player02가 한 건 올린다 — 참가자는 셋 다 같다.
    for day, headers in (("2026-07-01", h1), ("2026-07-02", h1), ("2026-07-03", h2)):
        await _create_match(
            client, headers, day,
            team1=[_slot("player01", "테란")], team2=[_slot("player02", "저그")], result="team1",
        )
    ...
    res = await client.get(
        "/api/game-results/stats", headers=h1, params={"memberIds": "player01,player02"}
    )
    by_id = {m["memberId"]: m for m in res.json()["members"]}
    assert by_id["player01"]["registered"] == 2
    assert by_id["player02"]["registered"] == 1

    # 기간을 좁히면 그 안에 올린 것만 센다.
    res = await client.get(
        "/api/game-results/stats", headers=h1,
        params={"memberIds": "player01,player02", "dateFrom": "2026-07-03", "dateTo": "2026-07-03"},
    )
    by_id = {m["memberId"]: m for m in res.json()["members"]}
    assert by_id["player01"]["registered"] == 0
    assert by_id["player02"]["registered"] == 1
