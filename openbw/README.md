...
| 파일 | 하는 일 |
|---|---|
| `bwdump.cpp` | 덤퍼 본체. `--tracks --bin`으로 조밀 이진 트랙을 낸다 (**베낀 것** — 아래) |
| `modern_replay.h` | 리마스터(1.21+) 리플레이 읽개 — `seRS`·zlib |
| `openbw-scr.patch` | OpenBW 원본에 얹는 우리 고침(리마스터 태그·그릇 한도·팀전 동맹 따위) |

## ⚠ 이 폴더의 세 파일은 프론트 리포에서 베껴 온다

원본은 프론트 리포의 `tools/openbw/`다. **고칠 일이 있으면 거기서 고친 뒤 여기로 통째로
복사하고, 두 리포를 같은 날 배포해야 한다.** Dockerfile이 굽는 것은 이쪽이라, 원본만
고치면 아무리 재분석해도 서버 바이너리는 옛것 그대로다.

실제로 그 일이 났다(지적: "재분석 여러번 했는데 가스 건물은 안붙어") — 가스 건물을
자취에 싣는 고침이 원본에만 들어가 있어, 서버는 24줄이 빠진 판을 굽고 있었다. 재분석은
제대로 돌고 있었고, 굽는 도구가 옛것이었을 뿐이라 로그로도 안 드러났다.

`bwdump.cpp` 맨 위에 **베껴 온 커밋**을 적어 둔다. 다음에 베낄 때 그 줄도 함께 갈면,
두 파일이 언제 갈렸는지가 파일 자신에 남는다.

    diff openbw/bwdump.cpp ../stargayte/tools/openbw/bwdump.cpp   # 갈렸는지 보는 법
